...
export function ForecastEvidence({ forecast, historical }: ForecastEvidenceProps) {
  ...
  return (
    <section aria-labelledby="forecast-evidence-heading" className="rounded-lg border border-border bg-card p-5">
      ...
      <p className="mt-1 text-sm leading-6 text-muted-foreground">A fixed-origin multi-step projection from the governed historical model artifact.</p>
      ...
      <p className="mt-5 border-t border-border pt-4 text-sm leading-6">The shaded range is a {forecast.interval_nominal_coverage * 100}% prediction interval, not a confidence interval. It is served for a fixed-origin multi-step projection and does not make a multi-horizon coverage claim.</p>
    </section>
  )
}
