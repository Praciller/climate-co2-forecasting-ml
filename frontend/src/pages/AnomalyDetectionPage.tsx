...
export function AnomalyDetectionPage({ anomalies, historical }: AnomalyDetectionPageProps) {
  ...
  return (
    <div className="space-y-10">
      <PageHeader title="Anomaly Detection" description="Compare two statistical signals over the packaged historical record. They are not verified climate events, sensor failures, or causal attributions." />
      <AnomalyEvidence anomalies={anomalies} />
      <dl className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4"><MetricDefinition label="Unique months" value={String(anomalies.length)} /><MetricDefinition label="Residual-threshold signal" value={String(residualCount)} detail="Mean absolute residual + 3 sigma" /><MetricDefinition label="Isolation Forest" value={String(isolationCount)} detail="3% contamination assumption" /><MetricDefinition label="Agreement" value={String(agreementCount)} detail={`${residualCount - agreementCount} residual only · ${isolationCount - agreementCount} Isolation Forest only`} /></dl>
