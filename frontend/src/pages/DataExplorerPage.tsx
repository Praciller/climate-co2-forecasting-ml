...
export function DataExplorerPage({ historical, modelInfo }: DataExplorerPageProps) {
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="grid gap-5 sm:grid-cols-3"><MetricCard label="Monthly rows" value={historical.length.toLocaleString()} detail={`${first?.date ?? '—'} to ${latest?.date ?? '—'}`} /><MetricCard label="Observed range · ppm" value={range} detail={`Source unit: ${dataset.unit}`} /><MetricCard label="Source observations" value={dataset.observed_values.toLocaleString()} detail={`${dataset.missing_values} missing of ${dataset.weekly_calendar_rows.toLocaleString()} calendar rows`} /></section>
